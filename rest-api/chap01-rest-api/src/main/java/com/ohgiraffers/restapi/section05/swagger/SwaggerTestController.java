...
public class SwaggerTestController {
    ...
    public ResponseEntity<ResponseMessage> findUserByNo(@PathVariable int userNo){
        ...
        return ResponseEntity.ok().headers(headers).body(responseMessage); // ok -> 빌더메소드
    }
    ...
    public ResponseEntity<Void> modifyUser(@PathVariable int userNo, @RequestBody UserDTO modifyInfo){
        ...
        return ResponseEntity
                .created(URI.create("/entity/users/" + userNo))     // put 의 상태코드
                .build();
    }
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "유효하지 않은 정보")
    })
