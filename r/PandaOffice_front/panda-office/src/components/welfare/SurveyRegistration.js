...
function SurveyRegistration() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch('http://localhost:8001/survey/categories');
        if (response.ok) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.message === 'The start date is already taken.') {
          alert('현재 진행중인 설문이 있습니다. 날짜를 변경해주세요.');
        } else {
  ...
  return (
    <div className="survey-registration-bg">
      ...
      <h1 className='survey-registration-title'>설문등록</h1>
        ...
        <div className="survey-area">
          <div className="survey-title-date">
            ...
            <select
              ...
              style={{padding:"8px 15px 8px 8px", marginTop:"10px", borderRadius:"5px"}}
            ...
            <ul>
              {newQuestion.options.map((option, oIndex) => (
                <li key={oIndex}>
                  ...
                  <span style={{margin:"7px 0 0 7px"}}>{['매우 만족', '약간 만족', '보통이다', '약간 불만족', '매우 불만족'][oIndex]}</span>
                </li>
              ))}
            </ul>
