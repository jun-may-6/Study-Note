...
public class Application {
    ...
    private static SearchCriteria inputSearchCriteria() {
        Scanner sc= new Scanner(System.in);
        System.out.println("검색하실 기준을 입력해주세요. (name or category)");
    ...
    private static List<Integer> createRandommenuCodeList() {
        ...
        Set<Integer> set = new HashSet<>();     //순서 x 중복 x add 로 중복된 값 넣으려 하면 저장 안됨
        while (set.size() < 5){
    ...
    private static void trimSubMenu() {
        ...
        do{
            ...
            System.out.println("2. 메뉴명 혹은 카테고리코드로 검색, 단 메뉴와 카테고리 둘 다 일치하는 경우도 검색하며, 검색조건이 없는 경우 전체검색");
            ...
            switch (no){
                case 1 : menuService.searchManuByCodeOrSearchAll(inputAllOrOne()); break;
