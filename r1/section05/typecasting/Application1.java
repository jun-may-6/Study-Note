package com.ohgiraffers.section05.typecasting;

public class Application1 {
    public static void main(String[] args) {
        /*필기
        데이터 형변환
        데이터 형변환이 필요한 이유
        자바에서 다른 타입끼리의 연산은 피연산자들이 모두 같은 타입인경우 실행할 수 있다.
        즉, 같은 데이터 타입끼리만 연산을 수행할 수 있다.
         */
        /*필기
        형변환의 종류와 규칙
        1. 자동 형변환 (묵시적 형변환) : 컴파일러가 자동으로 수행해주는 타입 변환
            1. 작은 자료형에서 큰 자료형으로는 자동 형변환이 된다.
            2. 정수는 실수로 자동 형변환 된다.
            3. 문자형은 int형으로 자동 형변환 된다.
            4. 논리형은 형변환 규칙에서 제외된다.
        강제 형변환 (명시적 형변환) : 형변환(cast) 연산자를 이용한 강제적으로 수행하는 형변환
            1. 자동형변환이 적용되지 않을 경우 강제 형변환이 필요하다.
         */

        byte bnum = 1;
        short snum = bnum;
        int inum = snum;
        long lnum = inum; //작은 자료형 -> 큰 자료형 문제없이 형변환됨

        int num1 = 10;
        long num2 = 20;
//        int result1 = num1 + num2; 자동 형변환에 의해 long 자료형이 되므로 int에 담을 수 없다.
        long result1 = num1 + num2;
        System.out.println("resule1 : " + result1);

        float fnum = 4.0f;
        double dnum = fnum;

        double result2= fnum + dnum;
        System.out.println("result2 : " + result2);

        /*필기.
        정수를 실수로 변경할 때 소수점 자리수가 없어도 실수형태로 표현이 가능하다.
        이 때 데이터 손실이 없기 때문에 자동 형변환이 가능하다.
         */

        long eight = 8;
        float four = eight;

        System.out.println("four : " + four);

        float result3 = eight + four;
        System.out.println("result3 : " + result3);

        char ch1 = 'a';
        int charNumver = ch1;

        System.out.println("charNumber : " + charNumver);

        /* 논리형은 형변환 규칙에서 제외된다.
        어느 자료형이든 boolean을 형변환 해서 담을 수 없다.
         */

        boolean isTrue = true;
//        byte b = isTrue;
//        int i - isTrue;
//        char c = isTrue;
//        double d = isTrue;


    }
}
