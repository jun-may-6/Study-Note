                ...
                <thead>
                    <tr>
                        <th onClick={setSelectAll}>
                            {selectAllTemplates ? <IoIosCheckbox /> : <IoIosCheckboxOutline />}
                        </th>
                        <th>제목</th>
                        <th>최종 수정자</th>
                        <th>최종 수정일</th>
                    ...
                    {currentFolder.documentList.map(doc => (
                        <tr key={doc.documentId}>
                            <td onClick={(e) => {
                                ...
                                {selectTemplates.includes(doc.documentId) ? <IoIosCheckbox /> : <IoIosCheckboxOutline />}
                            </td>
                            <td onClick={() => navigate(`/e_approval/document-template/${doc.documentId}`)}>{doc.title}</td>
                            <td onClick={() => navigate(`/e_approval/document-template/${doc.documentId}`)}>{`${doc.lastEditorName} ${doc.lastEditorJob}`}</td>
                            <td onClick={() => navigate(`/e_approval/document-template/${doc.documentId}`)}>{doc.lastEditDate}</td>
                            <td onClick={() => navigate(`/e_approval/document-template/${doc.documentId}`)}>{doc.status}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
