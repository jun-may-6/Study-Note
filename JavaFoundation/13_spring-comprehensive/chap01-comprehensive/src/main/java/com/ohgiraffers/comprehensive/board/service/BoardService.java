...
public class BoardService {
    ...
    public BoardDTO selectBoardDetail(Long boardNo) {
        ...
        /* 조회수 증가 로직 호출 */
        boardMapper.incrementBoardCount(boardNo);
    ...
    public void registThumbnail(BoardDTO board) {
        ...
        board.getAttachmentList().forEach(attachmentDTO -> {
            attachmentDTO.setRefBoardNo(board.getBoardNo());
    ...
    public BoardDTO selectThumbnailDetail(Long boardNo) {

        /* 조회수 증가 로직 호출 */
        boardMapper.incrementBoardCount(boardNo);

        /* 사진 게시글 조회 후 반환 */
        return boardMapper.selectThumbnailBoardDetail(boardNo);
    }
}
